...
/**
 * 
 * @param {宽度Number} width 
 * @param {高度Number} height 
 * @param {线的颜色String} lineColor 
 * @param {块的颜色String} blockColor 
 * @param {背景色渐变Array} lineargradientColor 
 * @returns {base64}
 */
export const svg = (width,height,lineColor,blockColor,lineargradientColor) => {
    let topimageWidth = width
    let topimageHeight = height
    ...
    `
    const base64Code = btoa(yui, { type: 'image/svg+xml' });
    return base64Svg = `url(data:image/svg+xml;base64,${base64Code})`
}